...
async function annotateDocument(text) {
    const systemPrompt = `Ти — експерт з аналізу документів. Твоя задача — надати дуже коротке резюме документа і перелік основних ризиків. Відповідь повинна починатися з "Резюме:" та містити список ризиків під заголовком "Ризики:". Якщо ризиків немає, зазнач це. Відповідь має бути коротким текстом, без HTML-тегів.`;
